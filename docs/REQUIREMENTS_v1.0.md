...
実測(2026-08-30、本番):初回表示 1.08秒(FCP 0.94秒)/ 診断書 0.43秒 /
F-3再計算 0.2ミリ秒(最重路線2,226点・車両2本+TCO)/ `/api/map` gzip 65KB /
バッチ 約3分。**全項目が目標を満たしている。**
...
**判断は12月**(チケット12で先送り)。最小公開は本番URLで生きており、いま作ると締切までの
更新を4ヶ月ぶん追従させ続けることになる。締切直前に「そのときの本番」を写す方が、
予備が本番より古いという事故も起きない。
...
| ワークフロー | トリガ | 内容 |
|---|---|---|
| `ci` | PR / main以外へのpush | 型チェック(`tsc --noEmit`)、Lint(ESLint。対象は`apps/web`)、ユニットテスト(ゴールデンテスト含む) |
| `deploy` | main への push / 手動 | `ci` を `uses:` で呼ぶ → ローカルD1/R2をシード → build → 本番D1マイグレーション → deploy → 疎通確認 |
...
4. ~~**GitHubリポジトリの作成とActions用シークレット登録**~~: `CLOUDFLARE_API_TOKEN`、`CLOUDFLARE_ACCOUNT_ID`
   — **完了(2026-08-30)**。`batch` / `deploy` の両ワークフローが本番で動いている
5. ~~**管理画面のシークレット2つを登録**~~(`BETTER_AUTH_SECRET` / `ADMIN_SETUP_TOKEN`)
   — **完了(2026-08-30)**。`wrangler secret list` で確認済み
6. ~~**管理者アカウントの初期登録**~~(F-7の初回のみ)— **完了(2026-08-30)**。
   `/admin/setup` は管理者が1人いる状態では開かないので、以降この入口は閉じている

**§11はすべて完了。** 以降まさゆきさんの手が要るのは、応募フォームの入力と紹介動画の撮影だけ。
