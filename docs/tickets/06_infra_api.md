...
- 回帰確認: 3路線(W1西部/W0-1駒場線/阿智55.6km)で電費・往復電池%・追加充電・累積上り・回生・
  ...
  診断書の印刷も変化なし(当時の測定は 1枚40 / 2枚6。ただしこの測り方は描画完了を待っておらず
  不安定だった — 正しくは 1枚39 / 2枚7。チケット07で測定手順を是正した)。
- マップ(F-1)は `/api/map` への切り替え後も実ブラウザで正常に表示されることをユーザーが確認済み。
  `/api/map` が同一のGeoJSON(46 features・11,717座標)を返すこと、凡例の集計(適34/条件付き12)、
  canvasの初期化、コンソールエラー0も確認済み。
  なお**MapLibreは `preserveDrawingBuffer: false` のためヘッドレスでは描画内容をキャプチャできない**
  (canvasが白いまま写る)。マップの見た目の検証は実ブラウザで行うこと。
