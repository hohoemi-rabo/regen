...
| 項目 | 目標 | 実測 |
|---|---|---|
| 全実行(取得あり・標高キャッシュ温) | 15分以内 | **80秒** |
| 更新なしの実行 | — | **8秒** |
| バンドル gzip 後 | 1.5MB以下 | **0.20MB** |
| 標高タイル | — | 159枚(`batch/.cache/dem/`) |
| R2オブジェクト / D1文 | — | 48 / 73 |

### 残り(ユーザー作業 — 要件§11-4)

Actionsは `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID` の登録まで動かない(未登録を確認済み)。
本番D1/R2への投入は `pnpm batch -- --remote` でいつでも可能。
