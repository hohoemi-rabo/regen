...
- **better-auth の React クライアントを使わない。** ログインもログアウトも `fetch` で叩く。
  ...
  **その `fetch` には `content-type: application/json` と本文を必ず付け、`res.ok` を確かめる。**
  付けないと better-auth は 415 を返し、Cookieが消えないまま「ログアウトしたつもり」になる。
  ログイン画面は有効なセッションを見つけて `/admin` へ送り返すので、
  **「ログアウトを押すと管理画面に戻る」という形でしか気づけない**(2026-08-30に発覚)
