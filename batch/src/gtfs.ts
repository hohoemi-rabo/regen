...
export function loadFeed(dir: string, feedName: string): FeedRoute[] {
  ...
  const stops = new Map(rd(dir, "stops.txt").map((s) => [s.stop_id, [parseFloat(s.stop_lat), parseFloat(s.stop_lon)] as [number, number]]));
  ...
  for (const [rid, ts] of byRoute) {
    ...
    out.push({ feed: feedName, routeId: rid, name, agency, mode, trips: ts.length, pts });
  }
  return out;
}
